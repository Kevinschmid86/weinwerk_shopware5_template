...
{block name='frontend_widgets_delivery_infos'}
    <div class="product--delivery">
        ...
        {elseif ($sArticle.sReleaseDate && $sArticle.sReleaseDate|date_format:"%Y%m%d" > $smarty.now|date_format:"%Y%m%d") || ($sArticle.sReleasedate && $sArticle.sReleasedate|date_format:"%Y%m%d" > $smarty.now|date_format:"%Y%m%d")}
            <link itemprop="availability" href="http://schema.org/PreOrder" />
            <p class="delivery--information">
				<span class="delivery--text delivery--text-more-is-coming">
                    <i class="delivery--status-icon delivery--status-more-is-coming"></i>
                    {s name="DetailDataInfoShipping"}{/s}
                    {if $sArticle.sReleaseDate}
                        {$sArticle.sReleaseDate|date:'date_long'}
                    {else}
                        {$sArticle.sReleasedate|date:'date_long'}
                    {/if}
				</span>
